...
!!! info
    The style guide is always a work in progress, and not all PyQHA code
    follows the rules. When modifying PyQHA, please fix the style violations
...
PyQHA uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/PyQHA.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the PyQHA directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to PyQHA have
